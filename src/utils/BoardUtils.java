...
public class BoardUtils {
	...
	public static Board buildBoard(String grid[], int columns, int rows, TokenMoveStrategy strategy) 
	{	
		Board board = null;
		...
		for (int row = board.getHeight() - 1; row >= 0 ; row--) {
			for (int column = 0; column < board.getWidth(); column++) {
				...
				switch (token) {
					...
					case ' ':	color = Token.NONE;
								break;
					case 'Y':	color = Token.YELLOW;
								break;
					case 'R':	color = Token.RED;
								break;
					default: 	return null;
				}
			...
			for (int j = 0; j < 7; j++) {
				switch (board.getSlot(j, i)) {
					...
					case NONE:	stringBuilder.append(' '); 
								break;
					case YELLOW: stringBuilder.append('Y'); 
								 break;
					case RED: stringBuilder.append('R'); 
								break;
				}
			}
			stringBuilder.append('|');
