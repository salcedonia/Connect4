...
public class ConnectFourGameTest extends TestCase {
	...
	Game game = new ConnectFourGame();
	game.startGame();
	assertEquals("Initial turn does not correspond to YELLOW",
		game.getTurn(), Token.YELLOW);
	...
	p1.setToken(new Position(1, 0));
	p1.setToken(new Position(0, 0));
	assertTrue("Fails when winner is YELLOW",
		p1.winner().equals(Token.YELLOW));
	...
	class GameOverYellow extends ConnectFourGame {
		...
		_board = BoardUtils.buildBoard(board, 7, 6,
